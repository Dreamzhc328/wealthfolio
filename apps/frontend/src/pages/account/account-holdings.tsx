      ...
      return (
        <div className="flex items-center justify-center py-16">
          <EmptyPlaceholder
            ...
            title={t("account.empty.holdingsTitle")}
            description={
              canEditHoldingsDirectly
                ? t("account.empty.holdingsManualDescription")
                : t("account.empty.holdingsConnectedDescription")
            }
          >
            {canEditHoldingsDirectly && (
    ...
    return (
      <div className="flex items-center justify-center py-16">
        <EmptyPlaceholder
          icon={<Icons.TrendingUp className="text-muted-foreground h-10 w-10" />}
          title={t("account.empty.holdingsTitle")}
          description={t("account.empty.transactionsDescription")}
        >
          <div className="flex flex-col items-center gap-3 sm:flex-row">
            <Button
              size="default"
            ...
            >
              <Icons.Plus className="mr-2 h-4 w-4" />
              {t("account.empty.addTransaction")}
            </Button>
            <Button
              size="default"
              variant="outline"
  ...
  return (
    <div>
      <div className="flex items-center justify-between py-4">
        <h3 className="text-lg font-bold">{t("account.holdingsTitle")}</h3>
        {canEditHoldingsDirectly && onAddHoldings && (
          <TooltipProvider>
            <Tooltip>
              ...
              <TooltipContent>
                <p>{t("account.updateHoldingsTooltip")}</p>
              </TooltipContent>
            </Tooltip>
          </TooltipProvider>
        )}
