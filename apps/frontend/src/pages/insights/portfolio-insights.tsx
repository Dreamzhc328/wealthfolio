  ...
  return (
    <div className="flex h-full w-full flex-col space-y-4 p-4">
      ...
      <div className="flex items-center justify-center py-8">
        <span className="text-muted-foreground text-sm">{t("holdings.insights.loading")}</span>
      </div>
    </div>
  );
};
...
export default function PortfolioInsightsPage() {
  ...
  const views: SwipablePageView[] = useMemo(
    () => [
      {
        value: "holdings",
        label: t("holdings.insights.tabs.holdings"),
        icon: Icons.PieChart,
        content: (
      ...
      {
        value: "performance",
        label: t("holdings.insights.tabs.performance"),
      ...
      {
        value: "income",
        label: t("holdings.insights.tabs.income"),
