  ...
  return (
    <div className="border-border bg-card shadow-xs rounded-lg border">
      ...
      <Collapsible open={assetsOpen} onOpenChange={setAssetsOpen}>
        <CollapsibleTrigger className="hover:bg-muted/50 flex w-full items-center justify-between px-4 py-3 transition-colors md:px-5">
          <div className="flex items-center gap-2">
            ...
            />
            <span className="text-sm font-semibold">{t("netWorth.assets")}</span>
          </div>
        ...
        <Collapsible open={liabilitiesOpen} onOpenChange={setLiabilitiesOpen}>
          <CollapsibleTrigger className="hover:bg-muted/50 flex w-full items-center justify-between border-t px-4 py-3 transition-colors md:px-5">
            <div className="flex items-center gap-2">
              ...
              />
              <span className="text-sm font-semibold">{t("netWorth.liabilities")}</span>
            </div>
      ...
      <div className="bg-muted/30 flex items-center justify-between border-t px-4 py-3 md:px-5">
        <span className="text-sm font-bold">{t("netWorth.netWorthLabel")}</span>
  ...
  if (isLoading) {
    return (
      <div className="w-full">
        <h2 className="text-md pb-2 font-semibold tracking-tight">
          {t("netWorth.composition")}
        </h2>
    ...
    return (
      <div className="flex min-h-[50vh] items-center justify-center p-8">
        <div className="text-center">
          ...
          <p className="text-destructive text-lg font-medium">{t("netWorth.loadFailed")}</p>
                ...
                <TooltipProvider>
                  <Tooltip>
                    ...
                    <TooltipContent side="bottom" className="max-w-[280px]">
                      <p className="mb-2 text-xs font-medium">{t("netWorth.stale.tooltipTitle")}</p>
                        ...
                        {netWorthData?.staleAssets.map((asset) => (
                          ...
                          >
                            ...
                            <span className="text-muted-foreground shrink-0">
                              {t("netWorth.stale.daysAgo", { days: asset.daysStale })}
                            </span>
                          </li>
                        ))}
                      </ul>
          ...
          <div className="grid grid-cols-1 gap-8 lg:grid-cols-3 lg:gap-20">
            ...
            <div className="lg:col-span-2">
              <div className="mb-4 mt-8 w-full lg:mt-0">
                ...
                {isLoading ? (
                  ...
                  >
                    <p className="text-sm">{t("netWorth.noAssets")}</p>
                    <Link
                      to="/holdings"
            ...
            <div className="space-y-4 lg:col-span-1">
                ...
                <div className="border-warning/30 bg-warning/5 rounded-lg border p-4 md:p-5">
                  <div className="flex items-start gap-3">
                    ...
                    <div className="min-w-0 flex-1">
                      <p className="text-sm font-medium">{t("netWorth.stale.title")}</p>
                      <p className="text-muted-foreground mt-1 text-xs">
                        {(netWorthData?.staleAssets.length ?? 0) === 1
                          ? t("netWorth.stale.descriptionSingular", {
                              count: netWorthData?.staleAssets.length ?? 0,
                            })
                          : t("netWorth.stale.descriptionPlural", {
                              count: netWorthData?.staleAssets.length ?? 0,
                            })}
                      </p>
              ...
              <div
                ...
                style={{ backgroundColor: THEME_COLOR_LIGHT }}
              >
                <p className="text-sm font-medium">{t("netWorth.manageAssets")}</p>
