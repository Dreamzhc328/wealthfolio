    ...
    return (
      <Card className="w-full border-0 bg-transparent shadow-none">
        <CardHeader className="py-2">
          <CardTitle className="text-md">{t("savingGoals.title")}</CardTitle>
        </CardHeader>
        <CardContent>
  ...
  if (!hasGoals) {
    return (
      <div className="flex flex-wrap gap-4 pb-4">
        <h2 className="text-md font-semibold">{t("savingGoals.title")}</h2>
        <Card className="border-border/50 bg-success/10 shadow-xs w-full">
          <CardContent className="px-4 py-6">
            <div className="text-center">
              <p className="text-sm">{t("savingGoals.empty")}</p>
              <Link
                to="/settings/goals"
                className="text-muted-foreground hover:text-foreground mt-2 inline-flex items-center gap-1 text-xs underline-offset-4 hover:underline"
              >
                {t("savingGoals.create")}
                ...
                <Tooltip key={goal.id}>
                  ...
                  <TooltipContent className="space-y-2">
                    ...
                    <ul className="list-inside list-disc text-xs">
                      <li>
                        {t("savingGoals.tooltip.progress")}:{" "}
                        <b>{formatPercent(currentProgress)}</b>
                      </li>
                      <li>
                        {t("savingGoals.tooltip.currentValue")}:{" "}
                        <b>
                          <AmountDisplay
                            value={currentValue}
                            currency={currency}
                      ...
                      <li>
                        {t("savingGoals.tooltip.targetValue")}:{" "}
                        <b>
                          <AmountDisplay
