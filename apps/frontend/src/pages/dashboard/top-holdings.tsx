...
function HoldingRow({
  ...
  const formattedQuantity = (holding.quantity ?? 0).toLocaleString(undefined, {
    maximumFractionDigits: 3,
  });
  const subtitle = parsedOption
    ? `${new Date(parsedOption.expiration + "T12:00:00").toLocaleDateString("en-US", { month: "short", day: "numeric" })} $${parsedOption.strikePrice} ${parsedOption.optionType}`
    : t("topHoldings.shares", { count: formattedQuantity });
  ...
  return (
    ...
    >
      ...
      <span className="text-muted-foreground text-xs">
        {extraCount > 0
          ? t("topHoldings.moreHoldings", { count: totalRemaining })
          : t("topHoldings.more", { count: totalRemaining })}
      </span>
  ...
  return (
    <Card className="w-full border-0 bg-transparent shadow-none">
      <CardHeader className="py-2">
        <CardTitle className="text-md">{t("topHoldings.title")}</CardTitle>
      </CardHeader>
      <CardContent>
        ...
        <Card className="border-border/50 bg-success/10 shadow-xs w-full">
          <CardContent className="px-4 py-6">
            <div className="text-center">
              <p className="text-sm">{t("topHoldings.empty")}</p>
              <Link
                to="/activities/manage"
                className="text-muted-foreground hover:text-foreground mt-2 inline-flex items-center gap-1 text-xs underline-offset-4 hover:underline"
              >
                {t("topHoldings.emptyAction")}
  ...
  return (
    <Card className="w-full border-0 bg-transparent p-0 shadow-none">
      <CardHeader className="flex flex-row items-center justify-between px-0 py-2">
        <CardTitle className="text-md">{t("topHoldings.titleShort")}</CardTitle>
          ...
          <Popover>
            ...
            >
              ...
              {(["total", "daily"] as const).map((v) => (
                ...
                >
                  {v === "total"
                    ? t("topHoldings.filter.totalReturn")
                    : t("topHoldings.filter.dailyChange")}
                  <span
                    className={cn(
              ...
              {(["value", "gain"] as const).map((v) => (
                ...
                >
                  {v === "value"
                    ? t("topHoldings.filter.totalValue")
                    : t("topHoldings.filter.gain")}
                  <span
                    className={cn(
                      "flex h-4 w-4 items-center justify-center rounded-full border-2",
