  ...
  return (
    ...
    >
      ...
      <div className="text-muted-foreground flex items-center gap-2 text-center text-xs sm:text-left">
        ...
        {totalRowCount === 0 ? (
          <span>{t("dataGrid.noActivities")}</span>
        ) : (
          <span>
            <span className="hidden sm:inline">{t("dataGrid.showing")} </span>
            <span className="text-foreground font-medium">{startRow}</span>
            <span className="hidden sm:inline"> {t("dataGrid.to")} </span>
            <span className="sm:hidden">-</span>
            <span className="text-foreground font-medium">{endRow}</span>
            <span className="hidden sm:inline"> {t("dataGrid.of")} </span>
            <span className="sm:hidden"> / </span>
            <span className="text-foreground font-medium">{totalRowCount}</span>
            <span className="hidden sm:inline"> {t("dataGrid.activitiesUnit")}</span>
          </span>
        )}
      </div>
      ...
      <div className="flex flex-wrap items-center justify-center gap-2 sm:flex-nowrap sm:gap-4">
        ...
        <div className="flex items-center gap-2">
          <span className="text-muted-foreground text-xs">
            <span className="hidden sm:inline">{t("dataGrid.rowsPerPage")}</span>
            <span className="sm:hidden">{t("dataGrid.show")}</span>
          </span>
          <Select
            value={`${pageSize}`}
            onValueChange={(value) => {
        ...
        <div className="flex items-center gap-1.5">
          <Input
            ...
            title={t("dataGrid.jumpToPage", { total: pageCount })}
          />
        ...
        <div className="flex items-center gap-0.5">
          ...
          >
            <span className="sr-only">{t("dataGrid.firstPage")}</span>
          ...
          <Button
            ...
            disabled={!canPreviousPage || isFetching}
            title={t("dataGrid.previousPage")}
          >
            <span className="sr-only">{t("dataGrid.previousPage")}</span>
          ...
          >
            <span className="sr-only">{t("dataGrid.lastPage")}</span>
