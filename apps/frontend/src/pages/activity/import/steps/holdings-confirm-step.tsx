  ...
  const handleImport = useCallback(() => {
    void (async () => {
      ...
      try {
        ...
        persistCreatedAssets(createdAssetIdsByKey);
        setPrepareError(
          error instanceof Error ? error.message : t("import.holdings.confirm.prepareFailed"),
        );
      } finally {
    ...
    <div className="flex flex-col gap-6">
      <Card>
        <CardHeader className="px-4 py-3">
          <CardTitle className="text-sm font-medium">
            {t("import.holdings.confirm.title")}
          </CardTitle>
        </CardHeader>
        <CardContent>
          <div className="space-y-4">
            <Alert>
              ...
              <AlertTitle>{t("import.holdings.confirm.important")}</AlertTitle>
              <AlertDescription>
                {t("import.holdings.confirm.importantDescription")}
              </AlertDescription>
            </Alert>
            ...
            <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
              <div className="bg-muted/30 rounded-lg p-3 text-center">
                ...
                <div className="text-muted-foreground text-xs">
                  {t("import.holdings.confirm.snapshots")}
                </div>
              </div>
              <div className="bg-muted/30 rounded-lg p-3 text-center">
                <div className="text-2xl font-bold">{totalPositions}</div>
                <div className="text-muted-foreground text-xs">
                  {t("import.holdings.confirm.positions")}
                </div>
              </div>
              <div className="bg-muted/30 rounded-lg p-3 text-center">
                <div className="text-2xl font-bold">{totalCashEntries}</div>
                <div className="text-muted-foreground text-xs">
                  {t("import.holdings.confirm.cashBalances")}
                </div>
              </div>
              <div className="bg-muted/30 rounded-lg p-3 text-center">
                <div className="text-2xl font-bold">{parsedRows.length}</div>
                <div className="text-muted-foreground text-xs">
                  {t("import.holdings.confirm.csvRows")}
                </div>
              </div>
            </div>
      ...
      {prepareError && (
        <Alert variant="destructive">
          ...
          <AlertTitle>{t("import.holdings.confirm.preparationFailed")}</AlertTitle>
      ...
      <div className="flex justify-end gap-3">
        <Button variant="outline" onClick={handleCancel} disabled={isImporting}>
          {t("import.holdings.confirm.cancel")}
        </Button>
        <Button onClick={handleImport} disabled={isImporting || snapshots.length === 0}>
          {isImporting ? (
            <>
              ...
              {isPreparingAssets
                ? t("import.holdings.confirm.preparing")
                : t("import.holdings.confirm.importing")}
            </>
          ) : (
            <>
              <Icons.Upload className="mr-2 h-4 w-4" />
              {snapshots.length === 1
                ? t("import.holdings.confirm.importSingle", { n: snapshots.length })
                : t("import.holdings.confirm.import", { n: snapshots.length })}
            </>
          )}
        </Button>
      </div>
