...
export function AccountSelection({ limit, accounts, deposits, isLoading }: AccountSelectionProps) {
  ...
  return (
    <div className="space-y-4">
      <h3 className="font-semibold">{t("contributionLimits.accountSelection.heading")}</h3>
                  ...
                  <div className="flex flex-col items-start">
                    ...
                    {isLoading ? (
                      <span className="text-muted-foreground text-xs">
                        {t("contributionLimits.accountSelection.loading")}
                      </span>
                    ) : accountDeposit ? (
                      <span className="text-muted-foreground text-xs font-light">
                        {formatAmount(accountDeposit.convertedAmount, deposits.baseCurrency)}{" "}
                        {t("contributionLimits.accountSelection.contributed")}
                      </span>
                    ) : null}
                  </div>
                </div>
      ...
      >
        {updateContributionLimitMutation.isPending
          ...
          : t("contributionLimits.accountSelection.save")}
      </Button>
    </div>
  );
}
