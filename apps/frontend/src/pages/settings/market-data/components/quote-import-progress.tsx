    ...
    <Card>
      <CardHeader>
        <CardTitle className="flex items-center gap-2">
          ...
          {t("marketData.quoteImport.progressTitle")}
        </CardTitle>
        <CardDescription>
          {isImporting
            ? t("marketData.quoteImport.importingQuotes")
            : t("marketData.quoteImport.importCompleted")}
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-4">
        <div className="space-y-2">
          <div className="flex justify-between text-sm">
            <span>{t("marketData.quoteImport.progress")}</span>
        ...
        <div className="flex gap-2">
          <Badge variant="outline" className="flex-1 justify-center">
            ...
            {t("marketData.quoteImport.importedBadge", { count: successfulRows })}
          </Badge>
          {failedRows > 0 && (
            <Badge variant="destructive" className="flex-1 justify-center">
              <Icons.XCircle className="mr-1 h-4 w-4" />
              {t("marketData.quoteImport.failedBadge", { count: failedRows })}
            </Badge>
          )}
        </div>
        ...
        {isImporting && onCancel && (
          <Button variant="outline" onClick={onCancel} className="w-full">
            ...
            {t("marketData.quoteImport.cancel")}
          </Button>
        )}
      </CardContent>
    </Card>
