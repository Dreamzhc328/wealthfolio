...
const MAPPING_META: {
  ...
  { field: "pricePath", labelKey: "marketData.preview.mapping.fields.price", color: "bg-emerald-500", required: true },
  { field: "datePath", labelKey: "marketData.preview.mapping.fields.asOf", color: "bg-sky-500", required: false },
  { field: "currencyPath", labelKey: "marketData.preview.mapping.fields.currency", color: "bg-amber-500", required: false },
  { field: "openPath", labelKey: "marketData.preview.mapping.fields.open", color: "bg-yellow-500", required: false },
  { field: "highPath", labelKey: "marketData.preview.mapping.fields.high", color: "bg-orange-500", required: false },
  { field: "lowPath", labelKey: "marketData.preview.mapping.fields.low", color: "bg-rose-500", required: false },
  { field: "volumePath", labelKey: "marketData.preview.mapping.fields.volume", color: "bg-violet-500", required: false },
];
  ...
  if (!template) {
    return (
      <span className="text-muted-foreground/60 font-mono text-xs italic">
        {t("marketData.preview.urlPlaceholder")}
      </span>
    );
  }
  ...
  if (isFetching) {
    return (
      <span className="text-muted-foreground inline-flex shrink-0 items-center gap-1.5 whitespace-nowrap rounded-full border px-3 py-1 text-xs">
        ...
        {t("marketData.preview.status.fetching")}
      </span>
    );
  }
  if (error || status?.ok === false) {
    return (
      <span className="text-destructive border-destructive/40 bg-destructive/10 inline-flex shrink-0 items-center gap-1.5 whitespace-nowrap rounded-full border px-3 py-1 text-xs">
        ...
        {t("marketData.preview.status.error")}
      </span>
    );
  }
  if (status?.ok) {
    return (
      <span className="text-success border-success/40 bg-success/10 inline-flex shrink-0 items-center gap-1.5 whitespace-nowrap rounded-full border px-3 py-1 text-xs">
        ...
        {t("marketData.preview.status.ok", { code: status.code })}
      </span>
    );
  }
  return (
    <span className="text-muted-foreground inline-flex shrink-0 items-center gap-1.5 whitespace-nowrap rounded-full border px-3 py-1 text-xs">
      ...
      {t("marketData.preview.status.idle")}
    </span>
  );
}
    ...
    <div className="flex min-h-[220px] flex-col items-center justify-center gap-3 rounded-xl border border-dashed p-6 text-center">
      ...
      <div className="space-y-1">
        <p className="text-muted-foreground text-sm">
          {t("marketData.preview.empty.instructionPrefix")}{" "}
          <span className="bg-muted rounded px-1.5 py-0.5 font-mono text-[11px]">
            {t("marketData.preview.fetch")}
          </span>{" "}
          {t("marketData.preview.empty.instructionSuffix")}
        </p>
        <p className="text-muted-foreground/70 text-xs">
          {t("marketData.preview.empty.hint")}
        </p>
      </div>
    </div>
  );
...
function HtmlTableResponse({
  ...
  const armedLabel = runtime.armedField
    ? t(MAPPING_META.find((m) => m.field === runtime.armedField)?.labelKey ?? "")
    : "";
    ...
    <div className="space-y-3">
      ...
      <div className="flex flex-wrap items-center gap-1">
        <span className="text-muted-foreground mr-1 text-[11px] font-medium uppercase tracking-wide">
          {t("marketData.preview.table.label")}
        </span>
        {tables.map((tbl, i) => (
          <button
            key={tbl.index}
            ...
            title={t("marketData.preview.table.tabTitle", {
        ...
        <span className="text-muted-foreground ml-auto text-xs">
          {runtime.armedField
            ? t("marketData.preview.table.headerHintArmed", {
                count: table.rowCount,
                field: armedLabel,
              })
            : t("marketData.preview.table.headerHint", { count: table.rowCount })}
        </span>
      </div>
                ...
                <th key={col.index} className="px-3 py-2 text-left">
                  ...
                  >
                    <span className="text-foreground text-xs font-medium">
                      {col.header || t("marketData.preview.table.columnFallback", { index: col.index })}
                    </span>
                    {col.role && (
                      <span
                        className={cn(
  ...
  return (
    <div className="space-y-2">
      <p className="text-muted-foreground text-xs">
        {runtime.armedField
          ? t("marketData.preview.csv.hintArmed", { count: rows.length, field: armedLabel })
          : t("marketData.preview.csv.hint", { count: rows.length })}
      </p>
      <div className="bg-background max-h-[420px] overflow-auto rounded-lg border">
        <table className="w-full text-xs">
          <thead className="bg-muted/40 sticky top-0">
...
function HtmlElementsResponse({ runtime }: { runtime: SourceRuntime }) {
  ...
  return (
    <div className="space-y-2">
      <p className="text-muted-foreground text-xs">
        {runtime.armedField
          ? t("marketData.preview.elements.hintArmed", {
              count: runtime.detectedElements.length,
              field: armedLabel,
            })
          : t("marketData.preview.elements.hint", { count: runtime.detectedElements.length })}
      </p>
      ...
      {hasMore && !showAll && (
        ...
        >
          {t("marketData.preview.elements.showMore", {
...
function CopyButton({ text, disabled }: { text: string; disabled?: boolean }) {
  ...
  return (
    <button
      ...
      onClick={handleCopy}
      disabled={disabled}
      title={copied ? t("marketData.preview.copiedTitle") : t("marketData.preview.copyUrlTitle")}
      ...
      <span className="text-foreground min-w-0 flex-1 truncate text-sm font-semibold">
        {display ?? (
          <span className="text-muted-foreground/60 italic">
            {t("marketData.preview.extracted.notReturned")}
          </span>
        )}
      </span>
...
function FieldMappingRow({
  ...
  return (
    ...
    >
      ...
      >
        {assigned ? value : t("marketData.preview.mapping.unassigned")}
      </span>
      {required ? (
        <span
          className={cn(
    ...
    <div className="flex h-full flex-col gap-4 overflow-y-auto p-5">
      ...
      <div className="flex items-start justify-between gap-3">
        <div className="space-y-0.5">
          <h3 className="text-base font-semibold">{t("marketData.preview.header")}</h3>
          <p className="text-muted-foreground text-xs">
            {t("marketData.preview.headerHint")}
          </p>
        </div>
        <StatusPill
      ...
      <section className="space-y-2">
        <div className="flex items-center justify-between">
          <Label className="text-muted-foreground text-[11px] font-medium uppercase tracking-wide">
            {t("marketData.preview.urlLabel")}
          </Label>
          <span className="text-muted-foreground text-[11px]">
            {isHistorical
              ? t("marketData.preview.modeHistorical")
              : t("marketData.preview.modeLatest")}
          </span>
        </div>
      ...
      <section className="space-y-2">
        <div className="flex flex-wrap items-end gap-2">
          <TestField label={t("marketData.preview.fields.testSymbol")} className="min-w-[140px] flex-1">
            <Input
              placeholder={t("marketData.preview.fields.testSymbolPlaceholder")}
              value={inputs.symbol}
              onChange={(e) => setInputs({ symbol: e.target.value })}
            />
          </TestField>
          {extraPlaceholders.isin && (
            <TestField label={t("marketData.preview.fields.isin")} className="w-28">
              <Input
          ...
          {extraPlaceholders.mic && (
            <TestField label={t("marketData.preview.fields.mic")} className="w-24">
              <Input
                placeholder="XLON"
                value={inputs.mic}
          ...
          {extraPlaceholders.currency && (
            <TestField label={t("marketData.preview.fields.currency")} className="w-20">
              <Input
                placeholder="USD"
          ...
          {isHistorical && (
            <>
              <TestField label={t("marketData.preview.fields.from")} className="w-36">
                <Input
                  type="date"
                  value={inputs.from}
              ...
              <TestField label={t("marketData.preview.fields.to")} className="w-36">
                <Input
                  type="date"
                  value={inputs.to}
      ...
      <section className="space-y-2">
        <div className="flex items-center justify-between">
          <Label className="text-muted-foreground text-[11px] font-medium uppercase tracking-wide">
            {t("marketData.preview.responseLabel", { format: formatLabel })}
          </Label>
          {runtime.detectedTables[0] && (
            <span className="text-muted-foreground text-[11px]">
              {t("marketData.preview.responseStats", {
                count: runtime.detectedTables[0].rowCount,
              ...
              <div>
                ...
                {/403|forbidden|denied/i.test(runtime.fetchError) && (
                  <p className="text-muted-foreground mt-1 text-xs">
                    {t("marketData.preview.errorHintForbidden")}
                  </p>
                )}
              </div>
            </div>
          ...
          {runtime.testResult.success ? (
            <div className="border-success/30 bg-success/5 divide-success/20 divide-y rounded-lg border">
              ...
              {values.datePath && (
                <ExtractedRow
                  color="bg-sky-500"
                  label={t("marketData.preview.mapping.fields.asOf")}
                  display={
              ...
              {values.openPath && (
                <ExtractedRow
                  color="bg-yellow-500"
                  label={t("marketData.preview.mapping.fields.open")}
                  display={
              ...
              {values.highPath && (
                <ExtractedRow
                  color="bg-orange-500"
                  label={t("marketData.preview.mapping.fields.high")}
                  display={
              ...
              {values.lowPath && (
                <ExtractedRow
                  color="bg-rose-500"
                  label={t("marketData.preview.mapping.fields.low")}
                  display={
              ...
              {values.volumePath && (
                <ExtractedRow
                  color="bg-violet-500"
                  label={t("marketData.preview.mapping.fields.volume")}
                  display={
              ...
              {values.currencyPath && (
                <ExtractedRow
                  color="bg-amber-500"
                  label={t("marketData.preview.mapping.fields.currency")}
                  display={
                    runtime.testResult.currency ? (
            ...
            <div className="border-destructive/30 bg-destructive/5 flex items-start gap-3 rounded-lg border p-3">
              ...
              <p className="text-muted-foreground min-w-0 flex-1 text-xs">
                {runtime.testResult.error ?? t("marketData.preview.extracted.extractFailed")}
              </p>
            </div>
          )}
        </section>
      ...
      <section className="space-y-2">
        <div className="flex items-center justify-between">
          <Label className="text-muted-foreground text-[11px] font-medium uppercase tracking-wide">
            {isHistorical
              ? t("marketData.preview.mapping.titleHistorical")
              : t("marketData.preview.mapping.titleLatest")}
          </Label>
          <span className="text-muted-foreground text-[11px]">
            {t("marketData.preview.mapping.hint")}
          </span>
        </div>
        ...
        <Collapsible open={moreFieldsOpen} onOpenChange={setMoreFieldsOpen}>
          <CollapsibleTrigger asChild>
            ...
            >
              ...
              />
              {t("marketData.preview.mapping.moreFields")}
            </button>
          </CollapsibleTrigger>
