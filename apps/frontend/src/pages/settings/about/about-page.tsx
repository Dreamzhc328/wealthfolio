...
export default function AboutSettingsPage() {
  ...
  const handleCopy = async (value: string, label: string) => {
    try {
      ...
      toast({ title: t("about.copied"), description: t("about.copiedDescription", { label }) });
    } catch (error) {
      toast({
        title: t("about.copyFailed"),
        description: t("about.copyFailedDescription", { label: label.toLowerCase() }),
        variant: "destructive",
      });
  ...
  return (
    <div className="space-y-6">
      <SettingsHeader heading={t("about.heading")} text={t("about.description")} />
      <Separator />
      ...
      <Card>
        <CardHeader className="flex flex-row items-center gap-4">
          ...
          <div className="flex flex-col">
            ...
            <CardDescription>
              {t("about.version", { version: version || t("about.versionUnavailable") })}
            </CardDescription>
          </div>
        </CardHeader>
        <CardContent className="space-y-6">
          <div className="space-y-4">
            <p className="text-muted-foreground text-sm">{t("about.tagline")}</p>
              ...
              <div className="grid gap-4">
                <div className="space-y-1">
                  <p className="text-muted-foreground text-xs uppercase tracking-wide">
                    {t("about.databasePath")}
                  </p>
                  <div className="flex items-center gap-2">
                    <p className="bg-muted text-muted-foreground flex-1 truncate rounded-md px-3 py-2 font-mono text-xs">
                      {dbPath || t("about.unavailable")}
                    </p>
                    <Button
                      variant="ghost"
                      size="icon"
                      disabled={!dbPath}
                      onClick={() => dbPath && handleCopy(dbPath, t("about.databasePath"))}
                    >
                      <Icons.Copy className="h-4 w-4" />
                      <span className="sr-only">{t("about.copyDb")}</span>
                    </Button>
                  </div>
                </div>
                <div className="space-y-1">
                  <p className="text-muted-foreground text-xs uppercase tracking-wide">
                    {t("about.logsDirectory")}
                  </p>
                  <div className="flex items-center gap-2">
                    <p className="bg-muted text-muted-foreground flex-1 truncate rounded-md px-3 py-2 font-mono text-xs">
                      {logsDir || t("about.unavailable")}
                    </p>
                    <Button
                      variant="ghost"
                      size="icon"
                      disabled={!logsDir}
                      onClick={() => logsDir && handleCopy(logsDir, t("about.logsDirectory"))}
                    >
                      <Icons.Copy className="h-4 w-4" />
                      <span className="sr-only">{t("about.copyLogs")}</span>
                    </Button>
                  </div>
                </div>
              </div>
