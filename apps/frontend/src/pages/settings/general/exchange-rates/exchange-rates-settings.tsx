  ...
  const columns: ColumnDef<ExchangeRate>[] = [
    ...
    },
    {
      ...
      cell: ({ row }) => {
        ...
        const names: Record<string, string> = {
          ...
          MANUAL: t("general.exchangeRates.sources.manual"),
          CUSTOM_SCRAPER: t("general.exchangeRates.sources.custom"),
          CUSTOMSCRAPER: t("general.exchangeRates.sources.custom"),
        };
    ...
    },
    {
      ...
      cell: ({ row }) => {
        ...
        return (
          <ActionConfirm
            confirmTitle={t("general.exchangeRates.delete.title")}
            confirmMessage={
              <>
                <p className="mb-2">
                  {t("general.exchangeRates.delete.confirmPrefix")}{" "}
                  <strong>{currencyPair}</strong>{" "}
                  {t("general.exchangeRates.delete.confirmSuffix")}
                </p>
                <p className="text-xs text-amber-600 dark:text-amber-400">
                  <Icons.AlertTriangle className="mr-1 inline h-3 w-3" />
                  {t("general.exchangeRates.delete.warning", { currency: rate.fromCurrency })}
                </p>
              </>
            }
          ...
          <Dialog open={isAddDialogOpen} onOpenChange={setIsAddDialogOpen}>
            <DialogTrigger asChild>
              <Button>
                ...
                {t("general.exchangeRates.addRate")}
              </Button>
            </DialogTrigger>
      ...
      <CardContent>
        {isLoadingRates ? (
          ...
          <div className="flex flex-col items-center justify-center py-10 text-center">
            ...
            <h3 className="mt-4 text-lg font-semibold">{t("general.exchangeRates.empty")}</h3>
