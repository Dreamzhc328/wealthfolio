...
export function RateCell({ rate, onUpdate }: RateCellProps) {
  ...
  const handleEdit = () => {
    if (!isManual) {
      toast({
        title: t("general.exchangeRates.rateCell.cannotEditTitle"),
        description: t("general.exchangeRates.rateCell.cannotEditDescription"),
        variant: "destructive",
      });
      return;
    }
  ...
  const handleSubmit = () => {
    ...
    if (isNaN(newRate) || newRate <= 0) {
      toast({
        title: t("general.exchangeRates.rateCell.invalidTitle"),
        description: t("general.exchangeRates.rateCell.invalidDescription"),
        variant: "destructive",
      });
      return;
    }
