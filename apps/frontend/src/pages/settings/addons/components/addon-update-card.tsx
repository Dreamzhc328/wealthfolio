...
export function AddonUpdateCard({
  ...
  const handleUpdate = async () => {
    if (!updateInfo.downloadUrl) {
      toast({
        title: t("addons.updateCard.notAvailableTitle"),
        description: t("addons.updateCard.notAvailableDescription"),
        variant: "destructive",
      });
      return;
    }
    ...
    try {
      ...
      toast({
        title: t("addons.updateCard.successTitle"),
        description: t("addons.updateCard.successDescription", {
          name: addonName,
      ...
      toast({
        title: t("addons.updateCard.failedTitle"),
        description:
          error instanceof Error ? error.message : t("addons.updateCard.failedDescription"),
        variant: "destructive",
      });
    } finally {
      setIsUpdating(false);
  ...
  const getUpdateBadgeText = () => {
    if (updateInfo.isCritical) return t("addons.updateCard.criticalBadge");
    if (updateInfo.hasBreakingChanges) return t("addons.updateCard.breakingBadge");
        ...
        <div className="flex-1 space-y-2">
          <div className="flex items-center gap-2">
            ...
            <h4 className="font-medium text-amber-900 dark:text-amber-100">
              {t("addons.updateCard.updateAvailable")}
            </h4>
            {getUpdateBadgeText() && (
            ...
            <Dialog open={showReleaseNotes} onOpenChange={setShowReleaseNotes}>
              <DialogTrigger asChild>
                <Button variant="ghost" size="sm">
                  ...
                  {t("addons.updateCard.releaseNotes")}
                </Button>
              </DialogTrigger>
              <DialogContent className="max-w-2xl">
                <DialogHeader>
                  <DialogTitle>
                    {t("addons.updateCard.releaseNotesTitle", {
                      name: addonName,
                      version: updateInfo.latestVersion,
                    })}
                  </DialogTitle>
                  <DialogDescription>
                    {t("addons.updateCard.whatsNew")}
                  </DialogDescription>
                </DialogHeader>
                  ...
                  <div className="space-y-4">
                    {updateInfo.releaseNotes && (
                      <div className="space-y-2">
                        <h4 className="font-medium">{t("addons.updateCard.releaseNotes")}</h4>
                    ...
                    {updateInfo.changelogUrl && (
                      <>
                        <Separator />
                        <div className="space-y-2">
                          <h4 className="font-medium">{t("addons.updateCard.fullChangelog")}</h4>
                          <a
                          ...
                          >
                            {t("addons.updateCard.viewChangelog")}
                          ...
                          <div className="flex items-start gap-2">
                            ...
                            <div>
                              <h5 className="text-destructive font-medium">
                                {t("addons.updateCard.breakingTitle")}
                              </h5>
                              <p className="text-destructive/80 text-sm">
                                {t("addons.updateCard.breakingDescription")}
                              </p>
                            </div>
                          </div>
                        </div>
                          ...
                          <div className="flex items-start gap-2">
                            ...
                            <div>
                              <h5 className="font-medium text-red-800 dark:text-red-200">
                                {t("addons.updateCard.criticalTitle")}
                              </h5>
                              <p className="text-sm text-red-700 dark:text-red-300">
                                {t("addons.updateCard.criticalDescription")}
                              </p>
                            </div>
                          </div>
                        </div>
          ...
          <Button onClick={handleUpdate} disabled={isUpdating || disabled} size="sm">
            {isUpdating ? (
              <>
                ...
                {t("addons.updateCard.updating")}
              </>
            ) : (
              <>
                <Icons.Download className="mr-1 h-3 w-3" />
                {t("addons.updateCard.update")}
              </>
            )}
          </Button>
        </div>
      ...
      {updateInfo.minWealthfolioVersion && (
        <div className="mt-3 rounded-md bg-amber-100 p-2 dark:bg-amber-900/20">
          <p className="text-xs text-amber-800 dark:text-amber-200">
            ...
            {t("addons.updateCard.requiresVersion", {
