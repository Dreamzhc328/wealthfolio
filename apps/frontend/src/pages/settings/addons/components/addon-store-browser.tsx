    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center">
        ...
        <div className="relative flex-1">
          ...
          <Input
            placeholder={t("addons.store.searchPlaceholder")}
            value={searchQuery}
        ...
        >
          <SelectTrigger className="w-full sm:w-[180px]">
            <SelectValue placeholder={t("addons.store.sortPlaceholder")} />
          </SelectTrigger>
          <SelectContent>
            <SelectItem value="popular">{t("addons.store.sortPopular")}</SelectItem>
            <SelectItem value="rating">{t("addons.store.sortRating")}</SelectItem>
            <SelectItem value="recent">{t("addons.store.sortRecent")}</SelectItem>
            <SelectItem value="name">{t("addons.store.sortName")}</SelectItem>
          </SelectContent>
        </Select>
        ...
        >
          <SelectTrigger className="w-full sm:w-[180px]">
            <SelectValue placeholder={t("addons.store.filterPlaceholder")} />
          </SelectTrigger>
          <SelectContent>
            <SelectItem value="all">
              {t("addons.store.filterAll", { count: counts.total })}
            </SelectItem>
            <SelectItem value="uninstalled">
              {t("addons.store.filterUninstalled", { count: counts.uninstalled })}
            </SelectItem>
          </SelectContent>
        </Select>
        ...
        <Button
          ...
          title={t("addons.store.refreshTooltip")}
        >
      ...
      <div className="space-y-4">
        ...
        <div className="flex items-center justify-between">
          <p className="text-muted-foreground text-sm">
            ...
            {searchQuery && (
              <>
                {" "}
                {t("addons.store.showingResultsForSearch", { query: searchQuery })}
              </>
            )}
          </p>
        ...
        {filteredAndSortedListings.length === 0 ? (
          <EmptyPlaceholder>
            ...
            <EmptyPlaceholder.Title>{t("addons.store.noResultsTitle")}</EmptyPlaceholder.Title>
            <EmptyPlaceholder.Description>
              {searchQuery
                ? t("addons.store.noResultsBodyWithSearch", { query: searchQuery })
                : t("addons.store.noResultsBodyWithFilters")}
            </EmptyPlaceholder.Description>
            <div className="flex gap-2">
              {searchQuery && (
                <Button variant="outline" onClick={() => setSearchQuery("")}>
                  {t("addons.store.clearSearchButton")}
                </Button>
              )}
              <Button variant="outline" onClick={() => setFilterBy("all")}>
                {t("addons.store.showAll")}
              </Button>
            </div>
          </EmptyPlaceholder>
        ) : (
      ...
      {searchQuery === "" && filterBy === "all" && !selectedTag && popularTags.length > 0 && (
        <div className="space-y-4">
          <Separator />
          <div>
            <h3 className="mb-3 text-lg font-semibold">{t("addons.store.categories")}</h3>
