...
const IncomePeriodSelector: React.FC<{
  ...
  const mobilePeriods = useMemo(
    () => [
      { value: "YTD" as const, label: t("income.periods.ytdShort") },
      { value: "LAST_YEAR" as const, label: t("income.periods.lastYearShort") },
      { value: "TOTAL" as const, label: t("income.periods.allShort") },
    ],
    [t],
  );
  return (
...
export default function IncomePage() {
  ...
  if (error || !incomeData) {
    return (
      <div>
        {t("income.loadFailed", { error: error?.message || t("income.unknownError") })}
      </div>
    );
  }
            ...
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              <CardTitle className="text-sm font-medium">
                {selectedPeriod === "TOTAL"
                  ? t("income.cards.allTimeIncome")
                  : selectedPeriod === "LAST_YEAR"
                    ? t("income.cards.lastYearIncome")
                    : t("income.cards.thisYearIncome")}
              </CardTitle>
                    ...
                    {periodSummary.yoyGrowth !== null ? (
                      <div className="flex items-center text-xs">
                        ...
                        <span className="text-muted-foreground ml-2 text-xs">
                          {t("income.cards.yoyGrowth")}
                        </span>
                      </div>
                    ) : (
                      <p className="text-muted-foreground text-xs">
                        {t("income.cards.cumulative")}
                      </p>
                    )}
                  </div>
                </div>
          ...
          <Card className="border-yellow-500/10 bg-yellow-500/10">
            ...
            <CardContent>
              ...
              <div className="flex items-center text-xs">
                ...
                <span className="text-muted-foreground ml-2 text-xs">
                  {t("income.cards.sinceLastPeriod")}
                </span>
              </div>
            </CardContent>
          </Card>
            ...
            <CardContent className="flex-1 overflow-auto">
              {topDividendStocks.length === 0 ? (
                <EmptyPlaceholder
                  ...
                  icon={<Icons.DollarSign className="h-10 w-10" />}
                  title={t("income.cards.noDividendTitle")}
                  description={t("income.cards.noDividendDescription")}
                />
              ) : (
                <div className="space-y-6">
                    ...
                    {(() => {
                      ...
                      const chartItems = [
                        ...
                        ...(otherTotal > 0
                          ? [
                              {
                                symbol: t("income.cards.other"),
                                companyName: t("income.cards.otherSources", {
