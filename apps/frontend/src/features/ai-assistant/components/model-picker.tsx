    ...
    return (
      <div className="flex items-center">
        <div className="text-muted-foreground flex items-center gap-1.5 px-3 text-xs">
          <span>{currentModel ? getDisplayName(currentModel.id) : t("modelPicker.noModel")}</span>
        </div>
        <ThinkingToggle />
      </div>
    );
      ...
      <Popover open={open} onOpenChange={setOpen}>
        <PopoverTrigger asChild>
          <Button
            ...
            aria-label={t("modelPicker.selectModelAria")}
          >
            <span>{currentModel ? getDisplayName(currentModel.id) : t("modelPicker.selectModel")}</span>
            <Icons.ChevronDown className="size-3 opacity-50" />
          </Button>
        </PopoverTrigger>
        <PopoverContent className="w-fit min-w-56 p-1.5" align="start" sideOffset={8}>
          <Command>
            <CommandEmpty>{t("modelPicker.noModelsAvailable")}</CommandEmpty>
            <CommandGroup>
            ...
            <CommandGroup>
              ...
              >
                ...
                <span className="whitespace-nowrap">{t("modelPicker.addModels")}</span>
              </CommandItem>
            </CommandGroup>
          </Command>
        </PopoverContent>
