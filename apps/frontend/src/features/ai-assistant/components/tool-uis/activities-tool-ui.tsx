  ...
  if (args?.displayMode === "compact" && parsed && !isLoading) {
    ...
    return (
      <CompactToolCard
        label={
          count === 1
            ? t("toolUI.activities.compactSingular", { count })
            : t("toolUI.activities.compactPlural", { count })
        }
      />
    );
  }
      ...
      <Card className="bg-muted/40 border-primary/10 w-full overflow-hidden">
        <CardHeader className="pb-2">
          <div className="flex flex-wrap items-start justify-between gap-2">
            <div>
              <CardTitle className="text-sm font-medium">{t("toolUI.activities.title")}</CardTitle>
            ...
            <Table>
              <TableHeader>
                <TableRow className="hover:bg-transparent">
                  <TableHead className="pl-4 text-xs">{t("toolUI.activities.headers.date")}</TableHead>
                  <TableHead className="text-xs">{t("toolUI.activities.headers.type")}</TableHead>
                  <TableHead className="text-xs">{t("toolUI.activities.headers.symbol")}</TableHead>
                  <TableHead className="text-right text-xs">{t("toolUI.activities.headers.qty")}</TableHead>
                  <TableHead className="text-right text-xs">{t("toolUI.activities.headers.price")}</TableHead>
                  <TableHead className="pr-4 text-right text-xs">{t("toolUI.activities.headers.amount")}</TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
    ...
    return (
      <Card className="bg-muted/40 border-destructive/30 w-full">
        <CardContent className="py-4">
          <p className="text-destructive text-sm">{t("toolUI.activities.loadFailed")}</p>
        </CardContent>
      </Card>
    );
  }
  ...
  if (isComplete && activitiesCount === 0) {
    return (
      <Card className="bg-muted/40 border-primary/10 w-full">
        <CardContent className="py-4">
          <p className="text-muted-foreground text-sm">{t("toolUI.activities.empty")}</p>
        </CardContent>
      </Card>
    );
  }
        ...
        <div className="flex flex-wrap items-start justify-between gap-2">
          <div>
            ...
            <p className="text-muted-foreground mt-1 text-xs">
              {activitiesCount === 1
                ? t("toolUI.activities.transactionsSingular", { count: activitiesCount })
                : t("toolUI.activities.transactionsPlural", { count: activitiesCount })}
            </p>
          </div>
