...
function AllocationContentImpl({ args, result, status }: AllocationContentProps) {
  ...
  const taxonomyName = parsed?.taxonomyName ?? t("toolUI.allocation.defaultName");
  ...
  if (args?.displayMode === "compact" && parsed && !isLoading) {
    return <CompactToolCard label={t("toolUI.allocation.compact")} />;
  }
      ...
      <Card className="bg-muted/40 border-primary/10 w-full overflow-hidden">
        <CardHeader className="pb-2">
          <div className="flex flex-wrap items-start justify-between gap-2">
            <div>
              <CardTitle className="text-sm font-medium">{t("toolUI.allocation.title")}</CardTitle>
    ...
    return (
      <Card className="bg-muted/40 border-destructive/30 w-full">
        <CardContent className="py-4">
          <p className="text-destructive text-sm">{t("toolUI.allocation.loadFailed")}</p>
        </CardContent>
      </Card>
    );
  }
  ...
  if (isComplete && categoryCount === 0 && holdingsCount === 0) {
    return (
      <Card className="bg-muted/40 border-primary/10 w-full">
        <CardContent className="py-4">
          <p className="text-muted-foreground text-sm">{t("toolUI.allocation.empty")}</p>
        </CardContent>
      </Card>
    );
  }
    ...
    return (
      <Card className="bg-muted/40 border-primary/10 w-full overflow-hidden">
        <CardHeader className="pb-2">
          <div className="flex flex-wrap items-start justify-between gap-2">
            <div>
              <CardTitle className="text-sm font-medium">{categoryName ?? t("toolUI.allocation.fallbackHoldings")}</CardTitle>
              <p className="text-muted-foreground mt-1 text-xs">
                {holdingsCount === 1
                  ? t("toolUI.allocation.holdingsSingular", { count: holdingsCount })
                  : t("toolUI.allocation.holdingsPlural", { count: holdingsCount })}
              </p>
            </div>
            <Badge variant="outline" className="text-xs">
              {t("toolUI.allocation.drillDown")}
            </Badge>
          </div>
          <div className="mt-2">
            ...
            <table className="w-full text-xs">
              <thead>
                <tr className="text-muted-foreground border-b text-left">
                  <th className="pb-2 font-medium">{t("toolUI.allocation.headers.symbol")}</th>
                  <th className="pb-2 font-medium">{t("toolUI.allocation.headers.name")}</th>
                  <th className="pb-2 text-right font-medium">{t("toolUI.allocation.headers.value")}</th>
                  <th className="pb-2 text-right font-medium">{t("toolUI.allocation.headers.weight")}</th>
                </tr>
              </thead>
              <tbody>
      ...
      <CardHeader className="pb-2">
        <div className="flex flex-wrap items-start justify-between gap-2">
          <div>
            ...
            <p className="text-muted-foreground mt-1 text-xs">
              {categoryCount === 1
                ? t("toolUI.allocation.categoriesSingular", { count: categoryCount })
                : t("toolUI.allocation.categoriesPlural", { count: categoryCount })}
            </p>
          </div>
