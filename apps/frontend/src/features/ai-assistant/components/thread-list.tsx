  ...
  if (threads.length === 0) {
    return (
      <div className="text-muted-foreground px-3 py-4 text-center text-sm">
        {search ? t("threadList.noMatches") : t("threadList.noConversations")}
      </div>
    );
  }
  ...
  return (
    <div className="flex flex-col gap-1">
      ...
      {unpinnedThreads.length > 0 && (
        <div className="flex flex-col gap-0.5">
          {pinnedThreads.length > 0 && (
            <div className="text-muted-foreground px-3 py-1 text-xs font-medium">{t("threadList.recent")}</div>
          )}
    ...
    <>
      {Array.from({ length: 5 }, (_, i) => (
        <div
          key={i}
          role="status"
          aria-label={t("threadList.loadingAria")}
          aria-live="polite"
      ...
      {isLoading ? (
        ...
        <div className="bg-muted/80 absolute inset-y-0 right-1 flex items-center gap-0.5 rounded-r-lg px-1 opacity-0 backdrop-blur-sm transition-opacity group-hover:opacity-100">
          <Button
            ...
            title={thread.isPinned ? t("threadList.unpin") : t("threadList.pin")}
          >
            {thread.isPinned ? (
          ...
          <ActionConfirm
            confirmTitle={t("threadList.deleteConfirmTitle")}
            confirmMessage={t("threadList.deleteConfirmMessage", {
              title: thread.title || t("threadList.deleteFallback"),
            })}
            confirmButtonText={t("threadList.deleteButton")}
            confirmButtonVariant="destructive"
            isPending={isDeleting ?? false}
            pendingText={t("threadList.deletePending")}
