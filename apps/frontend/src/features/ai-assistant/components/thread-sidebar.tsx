  ...
  const filteredThreads = useMemo(() => {
    ...
    if (searchQuery.trim()) {
      ...
      result = result.filter((thread) =>
        (thread.title || t("threadSidebar.newConversationFallback")).toLowerCase().includes(query),
      );
    }
    ...
    >
      ...
      >
        <div className="flex items-center gap-1.5">
          ...
          <span className="truncate font-medium">{thread.title || t("threadSidebar.newConversationFallback")}</span>
        </div>
        <div className="flex items-center gap-1.5">
  ...
  return (
    <div className={cn("flex h-full flex-col border-r", className)}>
      ...
      <div className="flex items-center justify-between border-b p-3">
        <h2 className="text-sm font-semibold">{t("threadSidebar.header")}</h2>
        <Button variant="ghost" size="icon" onClick={onNewThread} className="h-8 w-8">
          <Icons.Plus className="h-4 w-4" />
          <span className="sr-only">{t("threadSidebar.newConversation")}</span>
        </Button>
      </div>
      ...
      <div className="border-b p-2">
        <div className="relative">
          ...
          <Input
            type="search"
            placeholder={t("threadSidebar.searchPlaceholder")}
            value={searchQuery}
      ...
      <div className="flex-1 overflow-y-auto p-2">
        {isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center py-8 text-center">
            ...
            <p className="text-muted-foreground text-sm">{t("threadSidebar.noConversations")}</p>
            <p className="text-muted-foreground mt-1 text-xs">
              {t("threadSidebar.noConversationsHint")}
            </p>
          </div>
        ) : filteredThreads.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-8 text-center">
            <Icons.Search className="text-muted-foreground mb-2 h-6 w-6" />
            <p className="text-muted-foreground text-sm">{t("threadSidebar.noMatches")}</p>
            <p className="text-muted-foreground mt-1 text-xs">
              {filterTag ? t("threadSidebar.noMatchesHintTag") : t("threadSidebar.noMatchesHintSearch")}
            </p>
          </div>
        ) : (
          <div className="space-y-4">
            ...
            {unpinnedThreads.length > 0 && (
              <div>
                {pinnedThreads.length > 0 && (
                  <div className="text-muted-foreground mb-1 px-2 text-xs font-medium">{t("threadSidebar.recent")}</div>
                )}
      ...
      <Dialog open={renameDialogOpen} onOpenChange={setRenameDialogOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>{t("threadSidebar.renameDialogTitle")}</DialogTitle>
          </DialogHeader>
          <Input
            value={newTitle}
            onChange={(e) => setNewTitle(e.target.value)}
            placeholder={t("threadSidebar.renameDialogPlaceholder")}
            onKeyDown={(e) => {
              if (e.key === "Enter") {
                handleConfirmRename();
              }
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => setRenameDialogOpen(false)}>
              {t("threadSidebar.renameDialogCancel")}
            </Button>
            <Button onClick={handleConfirmRename} disabled={!newTitle.trim()}>
              {t("threadSidebar.renameDialogSave")}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={tagsDialogOpen} onOpenChange={(open) => !open && handleCloseTagsDialog()}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle>{t("threadSidebar.tagsDialogTitle")}</DialogTitle>
            <DialogDescription>{t("threadSidebar.tagsDialogDescription")}</DialogDescription>
          </DialogHeader>
          <div className="space-y-2">
            <Label htmlFor="tags">{t("threadSidebar.tagsLabel")}</Label>
            <InputTags
              id="tags"
              value={editingTags}
              onChange={handleTagsChange}
              placeholder={t("threadSidebar.tagsPlaceholder")}
            />
            <p className="text-muted-foreground text-xs">
              {t("threadSidebar.tagsHelp")}
            </p>
          </div>
          <DialogFooter>
            <Button onClick={handleCloseTagsDialog}>{t("threadSidebar.tagsDone")}</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      <AlertDialog open={deleteDialogOpen} onOpenChange={setDeleteDialogOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>{t("threadSidebar.deleteDialogTitle")}</AlertDialogTitle>
            <AlertDialogDescription>
              {t("threadSidebar.deleteDialogDescription", {
                title: selectedThread?.title ?? t("threadSidebar.deleteDialogFallback"),
              })}
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel>{t("threadSidebar.deleteCancel")}</AlertDialogCancel>
            <Button variant="destructive" onClick={handleConfirmDelete}>
              <Icons.Trash className="mr-2 h-4 w-4" />
              {t("threadSidebar.deleteConfirm")}
            </Button>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
